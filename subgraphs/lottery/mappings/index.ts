...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInCosmic);
...
export function handleTicketsPurchase(event: TicketsPurchase): void {
  ...
  if (user === null) {
    ...
    user.totalCosmic = ZERO_BD;
    user.block = event.block.number;
    user.timestamp = event.block.timestamp;
    user.save();
  }
  user.totalTickets = user.totalTickets.plus(event.params.numberTickets);
  user.totalCosmic = user.totalCosmic.plus(event.params.numberTickets.toBigDecimal().times(lottery.ticketPrice));
  user.save();
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  if (user !== null) {
    user.totalCosmic = user.totalCosmic.plus(toBigDecimal(event.params.amount));
    user.save();
  }
